#!/usr/bin/env python3
"""
SummarizedMosdepth.py (region-aware, robust) + integrated coverage plot

Usage:
  python SummarizedMosdepth.py --prefix test --thresholds 1,5,10,20,30
...
parser.add_argument(
    "--prefix", "-p", required=True, help="mosdepth prefix (e.g. 'Sample1')"
)
parser.add_argument(
    "--thresholds",
    "-T",
...
parser.add_argument(
    ...
    "-o",
    default=None,
    help="output CSV (default: <prefix>_coverage_summary.csv)",
)
parser.add_argument(
    "--patch-prefixes",
    default="KI,GL",
...
parser.add_argument(
    ...
    type=int,
    default=50000,
    help="bin size used when producing regions (for plot title)",
)
parser.add_argument(
    "--plot-out",
    default=None,
    help="path for coverage plot PNG (default: <prefix>_coverage_plot.png)",
)
...
try:
    summary_raw = pd.read_csv(
        summary_file, sep="\t", header=0, dtype={"chrom": str}, low_memory=False
    )
...
summary["length"] = (
    pd.to_numeric(summary["length"], errors="coerce").fillna(0).astype(np.int64)
)
summary["bases"] = (
    pd.to_numeric(summary["bases"], errors="coerce").fillna(0).astype(np.int64)
)
summary["mean"] = (
    pd.to_numeric(summary["mean"], errors="coerce").fillna(0.0).astype(float)
)
...
if len(thr_list_user) == threshold_count:
    ...
    if has_region:
        thr_names = ["chrom", "start", "end", "region"] + [
            f"count_{v}X" for v in thr_list_user
        ]
    else:
    ...
    if has_region:
        thr_names = ["chrom", "start", "end", "region"] + [
            f"count_col{i + 1}" for i in range(threshold_count)
        ]
    else:
        thr_names = ["chrom", "start", "end"] + [
            f"count_col{i + 1}" for i in range(threshold_count)
        ]
    print(
        f"WARNING: provided --thresholds length ({len(thr_list_user)}) does not match "
        f"thresholds file columns ({threshold_count}). Using file-order names: {thr_names[3:]}",
        file=sys.stderr,
    )
...
thr_agg = (
    thr.groupby("chrom").sum(numeric_only=True).drop(columns=drop_cols, errors="ignore")
)
...
thr_cols = [
    c for c in merged.columns if isinstance(c, str) and (c.startswith("count_"))
]
thr_cols = sorted(
    thr_cols, key=col_sort_key
)  # sort by number inside column name if present
...
use_chr_prefix = any(
    isinstance(x, str) and x.startswith("chr")
...
if len(patch_rows) > 0:
    ...
    p_bases = (
        int(merged.loc[patch_rows, "bases"].sum()) if "bases" in merged.columns else 0
    )
...
try:
    ...
    print(
        f"Regions file '{regions_file}' not found — skipping coverage plot.",
        file=sys.stderr,
    )
...
regs["mean_cov"] = (
    pd.to_numeric(regs["mean_cov"], errors="coerce").fillna(0.0).astype(float)
)
...
if not present_canon:
    print(
        "No canonical chromosomes found in regions file. Skipping coverage plot.",
        file=sys.stderr,
    )
    sys.exit(0)
...
if "Chromomosome" in df_out.columns and "Total" in df_out["Chromomosome"].values:
    avg_total = float(
        df_out.loc[df_out["Chromomosome"] == "Total", "Avg Coverage (X)"].iloc[0]
    )
else:
    # fallback: use merged row 'total' or genome mean
    avg_total = (
        float(merged.loc["total", "mean"])
        if "total" in merged.index
        else float(merged["mean"].mean())
    )
...
ax.axhline(
    avg_total, color="gray", linestyle="--", linewidth=1, label=f"avg={avg_total:.2f}"
)
