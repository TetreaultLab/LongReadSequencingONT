...
for s in samples:
    ...
    # merge
    print("--> Merge")
    cmd = ["samtools", "merge", "-f", "--threads", "7", "-o", str(output_file)] + bam_files_str
    print(" ".join(cmd))
    subprocess.run(cmd, check=True)

    # sort
    print("--> Sort + Index")
    cmd2 = ["samtools", "sort", "--threads", "7", "-m", "2G", "-o", output + "/" + s + "_sorted.bam", output + "/" + s + ".bam"]
    print(" ".join(cmd2))
    subprocess.run(cmd2, check=True)
    
    # index
    print("--> Index")
    cmd3 = ["samtools", "index", "--threads", "7", "-o", output + "/" + s + "_sorted.bam.bai", output + "/" + s + "_sorted.bam"]
    print(" ".join(cmd3))
